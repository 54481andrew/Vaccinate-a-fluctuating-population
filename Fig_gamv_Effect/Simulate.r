## This script varies gamv - this is only
## for use in the response to the reviewers,
## and is not a figure in the main text.
...
## Simulation time: 154 core minutes

rm(list = ls(all=TRUE))
datname  <- "Fig_S" ###Name of the data file
parmat <- expand.grid(d = c(1/(365*2.5)),
                      gamv = c(100, 1/7, 1/14, 1/30, 1/60),
                      rho = c(0.5), tv = seq(0,365,length.out = 100),
                      tb = c(120), T = 365, NPeak = 1000)
...
starttime <- Sys.time()
sollist <- mclapply(X=1:npars, FUN=mcl.fun, mc.cores=detectCores() - 1)
