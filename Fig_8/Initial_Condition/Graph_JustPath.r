...
parmat <- expand.grid(d = c(1/(4*365)),
                      gamv = 1/14,
                      Nv = 0,
                      tb = 2*30.4, T = 365, NPeak = 1107,
                      gamp = c(1/(4*365)), pmu = 1,
		      Rp = c(1.5),
                      tv = 0, type = c('f'))
...
for(i in wi.graph){
    ...
    ## Population Size
    setEPS()
    fn <- 'Figure_Badger.eps'
    postscript(file = fn, width = 5,
        height = 4)
    par(mai = c(0.75, 0.75, 0.25, 0.25))
    plot(NA, xlim = c(0,365), ylim = c(0,1), xaxt = 'n', xlab = '', ylab = '')
    tb <- unique(parmat$tb)
    polygon(x = c(0,tb,tb,0), y =c(-100,-100,1000,1000), col = 'gray90',
            border = NA)
    ...
    legend(x = 'topright',legend = c('Total Pop Size', 'Susceptible', 'TB Infected', 'Recovered'),
           col = c('black', 'blue', 'red', 'gray'), pch = c(NA,2,2,2), lwd = 1.5, cex = 0.8, bg = 'white', ncol = 2)
