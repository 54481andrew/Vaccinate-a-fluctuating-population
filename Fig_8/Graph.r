## This script produces a 1x2 paned figure from the Gillespie simulation
## data.
...
parmat$pmu <- 1 ## Simulated virulence
...
## Loop through each row of gridvals, and produce a 1x2 figure
...
for(jj in 1:NValues){
    ...
    for(tvi in 1:ntvvals){
        ...
        for(breaki in 1:nsims){
            ...
            ## end before the vaccination has begun are included in the data. This
            ## is pathogen elimination due to stochasticity. We filter these out
            ## at the bootstrapping stage (below).
...
fn <- 'Figure_8.eps'
setEPS()
postscript(file = fn, height = 2.55, width = 5)
par(mfrow = c(1,2), omi = c(0,0,0,0), mai = c(0.5, 0.4, 0.3, 0.05))
for(jj in 1:NValues){
    ...
    mtext(side = 1, text = 'Time of Vaccination', line = 1.6, cex = cx)
