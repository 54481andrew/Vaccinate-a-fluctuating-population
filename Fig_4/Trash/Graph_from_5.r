...
for(i in 1:(nrow(ParMat)-1)){
    ...
    tv <- ParMat$tv[i]
    tv.first <- tv + TBurnIn
    plot(NA, , xlim = c(TBurnIn-2*365, TBurnIn + 5*365), ylim = c(0,35000), xaxt = 'n', yaxt = 'n',
         xlab = '', ylab = '')
    for(yi in 1:20){
        time <- yi*365
...
##as.expression(bquote('Density ( x10'^'3'~'per km'^'2'~')')),
          arrows(x0 = tv.first + seq(0,4380, by = 365), y0 = 35000, y1 = 31000,
