...
out <- data.frame(lsoda(y = y0, times = seq(0,10*365,by=1),
                        func = rhs.dens.full, parms = parmat[1,],
                        maxsteps = 100000))
...
for(jj in 1:NValues){
    ...
    text(x = -10 + seq(0,365, length.out = 13), -0.525, labels = month.abb[c(6:12,1:6)],
         srt = 65, pos = 1, xpd = TRUE, cex = cx)
    axis(side = 2, labels = TRUE, at = seq(-0.5,1,by = 0.25), cex.axis = cx, padj = 1)
    ...
    axis(side = 2, labels = TRUE, at = seq(0,1,by = 0.25), cex.axis = cx, padj = 1)
    axis(side = 1, labels = FALSE, at = seq(0,365, by = 365/12) )
    text(x = -10 + seq(0,365, length.out = 13), -0.525, labels = month.abb[c(6:12,1:6)],
         srt = 65, pos = 1, xpd = TRUE, cex = cx)
    mtext(side = 1, text = 'Time of Vaccination', line = 1.8, cex = cx)
