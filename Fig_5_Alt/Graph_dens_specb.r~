...
name <- paste('dens_ls',round(1/unique(ParMat$d)), sep = '')
ParMat$b     <- with(ParMat,
                     NPeak*d/(exp(d*(T-tb))*(exp(d*tb)-1)/(exp(d*T)-1)))
##ParMat$Bp    <- with(ParMat, Rp*(d+gamp)) ## if frequency dependent
ParMat$Bp    <- with(ParMat, Rp*(d+gamp) / (b*tb/(d*T))  ) ## if density dependent
...
for(i in 1:nrow(ParMat)){
    ...
    out1 <- data.frame(lsoda(y = y0, times = seq(0,TBurnIn,by=1),
                             func = rhs.dens.full, parms = ParMat[i,]))
    y0 = out1[nrow(out1), -1]
    y0 = as.numeric(y0)
    ParMat$Nv[i] <- NVacc
    names(y0) = c('S','Iv','Ip','V','P')
...
##    fn <- paste('Figures/Figure_S3',type, sep='')
    fn <- paste('Figures/Figure_S3_', name,type, sep='')
...
for(type in '.png'){##c('.eps')){
    fn <- paste('Figures/Figure_5_', name,type, sep='')
    ##    setEPS()
...
##    abline(v = c(time, time + ParMat$tb[i]), lty = 3, lwd = 0.25)
    ...
    if(i < nrow(ParMat)){
        arrows(x0 = tv.first + seq(0,2*365, by = 365), y0 = 230, y1 = 200,
