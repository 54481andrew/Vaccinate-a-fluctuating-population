...
for(jj in 1:NValues){
    ...
    PlotMaxMat <- matrix(,nrow = ntvvals, ncol = nfocYears)
...
### loop and collect baseline data
    ...
    meansimbef  <- mean(simdat.base$Ip) ## Get average pathogen abundance
    ...
    maxsimaft <- matrix(, nrow = nsims, ncol = nfocYears)
    ...
    for(breaki in 1:nsims){
        ...
        for(foci in 1:length(focYears)){
            ...
            maxsimaft[breaki,foci] <-  max(simdati$Ip[wi.focYear], na.rm = T)
    ...
    print(paste("************************** ", jj, " *************************", sep = ''), quote = FALSE)
}
...
for(type in c('.eps')){
    fn <- paste('Figure_8',type, sep='')
    setEPS()
    postscript(file = fn, height = 2.5, width = 5)
par(mfrow = c(1,2), mai = c(0.6, 0.4, 0.25, 0.05))
for(jj in 1:NValues){
    PlotMeanMat <- MeanMatList[[jj]]
    PlotExtMat <- ExtMatList[[jj]]
    ## Proportional Reduction
plot(NA, xlim = c(0,365), ylim = c(0,1.2), xaxt = 'n', yaxt = 'n', xlab = '', ylab = '')
polygon(x = c(0,tbval,tbval,0), y =c(-100,-100,1.2*1.03,1.2*1.03), col = 'gray90',
        border = NA)
matlines(tvvals, PlotMeanMat, pch = 19, cex = cx, type = 'b',
        lty = 1, lwd = 2)
...
legend(x = 'topright',legend = 1:5, col = 1:5, pch = 19,
       ...
       cex = cx*0.8, bg = 'white', ncol = 1)
    rhoval <- gridvals$Rhoval[jj]
    legend(x =  'topleft',legend = as.expression(bquote(.(c(250,500)[jj])~' Vaccinations')),
           cex = cx*0.8, bg = 'white', xjust = 3)
    axis(side = 1, labels = FALSE, at = seq(0,365, by = 365/12) )
text(x = -10 + seq(0,365, length.out = 13), -0.125, labels = month.abb[c(2:12,1:2)],
