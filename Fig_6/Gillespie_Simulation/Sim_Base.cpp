...
int NParSets = tbVals.size()*R0pVals.size()*gampVals.size()*dVals.size()*npeakVals.size();
  ...
  for(int Par = 0; Par < NParSets; Par++) //Loop through parameter sets
    {
      ...
      double StartTime = 0;
      double EndTime = 1.0*365.0; //Maximum time to run each simulation trial
	...
	whichmin, unif, tmodT;
      ...
      d = ParMat[Par][2]; //d
      Bp = ParMat[Par][3]; //Bp
      Nv = 0; // Nv is 0 in Sim_Base simulations
      tv = ParMat[Par][5]; //tv
